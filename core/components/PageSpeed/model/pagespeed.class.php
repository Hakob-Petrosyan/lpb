...
//
    class PageSpeed {
        ...
        */
        ...
        const MODX_OPTION = [ 'assets_path', 'assets_url', 'core_path', 'http_host', 'site_url', 'upload_files', 'url_scheme' ];
        ...
        */
        ...
        const PROPERTIES_CACHE_IMAGE = [ 'convert', 'quality' ];
        const PROPERTIES_CACHE_STYLESHEET = [ 'convert', 'display', 'quality' ];
        ...
        */
        ...
        public function filename( string $url, string $base_path = NULL, string $base_url = NULL ) {
            if( parse_url( $url, PHP_URL_SCHEME ) === 'data' )
                return;
            if( is_file( $filename = str_replace( '/', DIRECTORY_SEPARATOR, preg_replace( sprintf( PageSpeed::FORMAT_PCRE_START_OF_STRING, preg_quote( $base_url ?? $this->properties[ 'site_url' ], PageSpeed::PCRE_DELIMITER ) ), $base_path ?? MODX_BASE_PATH, strtok( $url, '?' ) ) ) ) )
                return $filename;
        }
        ...
        public function parseCSSBlockList( Sabberworm\CSS\CSSList\CSSBlockList $CSSBlockList, string $base_url ) {
            foreach( $CSSBlockList->getContents() as $CSSBlock ) {
                if( $CSSBlock instanceof Sabberworm\CSS\Property\Import ) {
                    ...
                    if( strtok( $base, '#' ) === strtok( $url = $this->url( $atRuleArgs[ 0 ]->getUrl()->getString(), $base_url ), '#' ) )
                            ...
                            if( $this->properties[ 'WEBP' ] && $this->properties[ 'convert' ] !== 'disable' ) {
                                foreach( $this->DOMXPath->query( PageSpeed::XPATH_SRC ) as $DOMNode )
                                    if( in_array( $extension = $this->extension( $DOMNode->nodeValue ), PageSpeed::EXTENSION_IMAGE ) )
                                        $DOMNode->nodeValue = $this->subresource( $this->url( $DOMNode->nodeValue ), FALSE, $extension );
                                foreach( $this->DOMXPath->query( PageSpeed::XPATH_SRCSET ) as $DOMNode ) {
                                    ...
                                    $DOMNode->nodeValue = implode( ',', $srcset );
                                }
                            }
                            foreach( $this->DOMXPath->query( PageSpeed::XPATH_LINK ) as $DOMNode ) {
                                $as = strtolower( $DOMNode->getAttribute( 'as' ) );
                                if( $filename = $this->filename( $href = $DOMNode->getAttribute( 'href' ) ) )
                                    $DOMNode->setAttribute( 'href', strtok( $href, '?' ) . $this->version( $filename ) );
                                elseif( in_array( $as, PageSpeed::HTML_ATTRIBUTE_VALUE_AS ) &&
        ...
        public function subresource( string $url, bool $return, string $extension = NULL, string &$integrity = NULL ) {
            $extension = $extension ?? $this->extension( $url );
            if( $this->properties[ 'WEBP' ] && in_array( $extension, PageSpeed::EXTENSION_IMAGE ) ) {
                if( $this->properties[ 'convert' ] === 'dynamic' )
                    return $return ? $this->convert( $this->data( $url ) ) : htmlspecialchars( sprintf( $this->properties[ 'format_url_convert' ], $this->key, $url ) ) . parse_url( $url, PHP_URL_FRAGMENT );
                if( in_array( $this->properties[ 'convert' ], PageSpeed::PROPERTIES_CACHE_CONVERT ) )
                    $extension = 'webp';
            }
            if( in_array( $extension, $this->properties[ 'upload_files' ] ) ) {
                if( in_array( $extension, PageSpeed::EXTENSION_IMAGE_WEBP ) )
                ...
                if( $this->properties[ 'convert' ] === 'persistent' && in_array( $extension, PageSpeed::EXTENSION_IMAGE_WEBP ) && $filename = $this->filename( $url ) ) {
                    $basename = pathinfo( $path = parse_url( $url, PHP_URL_PATH ), PATHINFO_FILENAME ) . '.' . $extension;
                    ...
                    $basename = strlen( $extension ) ? substr( $hash, PageSpeed::CACHE_SPLIT_LENGTH * PageSpeed::CACHE_SPLIT_LEVELS ) . ( $minify ? '.min.' : '.' ) . $extension : substr( $hash, PageSpeed::CACHE_SPLIT_LENGTH * PageSpeed::CACHE_SPLIT_LEVELS );
                ...
                if( $data = $this->data( $url ) ) {
                    if( $this->properties[ 'integrity' ] && isset( $integrity ) && ! $this->integrity( $data, $integrity ) )
                        ...
                        if( $this->properties[ 'WEBP' ] && in_array( $this->properties[ 'convert' ], PageSpeed::PROPERTIES_CACHE_CONVERT ) && in_array( $extension, PageSpeed::EXTENSION_IMAGE_WEBP ) )
