textAdvs.grid.Default = function (config) {
    config = config || {};

    if (typeof(config['multi_select']) != 'undefined' && config['multi_select'] == true) {
        config['sm'] = new Ext.grid.CheckboxSelectionModel();
    }
    config['cls'] = (config['cls'] || 'main-wrapper') + ' txa-grid';
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('txa-grid__row_disabled');
                }
                return cls.join(' ');
            },
        },
    ...
    textAdvs.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(textAdvs.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = textAdvs.utils.getMenu(row.data['actions'], this, ids);
    ...
    _doAction: function (action, action_prefix, confirm, check_id) {
        ...
        if (confirm) {
            MODx.msg.confirm(Ext.apply({}, params, {
                title: ids.length > 1
                    ? _('txa_button_' + action + '_multiple')
                    : _('txa_button_' + action),
                text: _('txa_confirm_' + action),
            }));
        } else {
...
Ext.reg('txa-grid-default', textAdvs.grid.Default);